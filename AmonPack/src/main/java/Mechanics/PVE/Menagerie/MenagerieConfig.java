...
public class MenagerieConfig {
    ...
    public List<Menagerie> LoadMenageriesUponStart(List<String> Worlds){
        List<Menagerie>listofM=new ArrayList<>();
        for (String wname : Worlds){
        for (FileConfiguration config:AmonPackPlugin.GetMenagerieConfig()){
            for(String MenagerieName : Objects.requireNonNull(config.getConfigurationSection("Menagerie")).getKeys(false)) {
                ...
                if(BaseWorld!=null&&wname.contains(BaseWorld)){
                    BaseWorld=wname;
                ...
                for(String EncounterName : Objects.requireNonNull(config.getConfigurationSection("Menagerie."+MenagerieName+".Encounters")).getKeys(false)) {
                    ...
                    for(String ObjectiveName : Objects.requireNonNull(config.getConfigurationSection("Menagerie."+MenagerieName+".Encounters."+EncounterName+".Objectives")).getKeys(false)) {
                        ...
                        for (String OEffectName : Objects.requireNonNull(config.getConfigurationSection(Path + ".Effects")).getKeys(false)) {
                            ...
                            ObjectiveEffect objeff=parseEffect(Path + ".Effects." + OEffectName,BaseWorld,config);
                            if(objeff!=null){
                                if(Time>0){
                        ...
                        for(String OCon : Objects.requireNonNull(config.getConfigurationSection(Path+ ".Conditions")).getKeys(false)) {
                            Conditions.add(parseCondition(Path + ".Conditions." + OCon,BaseWorld,config));
                        }
                        ...
                        ItemStack itemdrop =parseItemStack(Path + ".DropItems",config);
                        if(itemdrop!=null){
    ...
    public Menagerie MenagerieFromWorldName(String World){
            for (FileConfiguration config:AmonPackPlugin.GetMenagerieConfig()){
                for(String MenagerieName : Objects.requireNonNull(config.getConfigurationSection("Menagerie")).getKeys(false)) {
                    String BaseWorld = config.getString("Menagerie."+MenagerieName+".Base_World_Name");
                    if(BaseWorld!=null&&World.contains(BaseWorld)){
                        BaseWorld=World;
                        List<Integer> CenterX = config.getIntegerList("Menagerie."+MenagerieName+".Center_Location");
                        int RangeX = config.getInt("Menagerie."+MenagerieName+".Range_X");
                        int RangeZ = config.getInt("Menagerie."+MenagerieName+".Range_Z");
    ...
    private ObjectiveEffect parseEffect(String path,String world, FileConfiguration config) {
        Location effectLocation = parseLocation(path + ".EffectLocation",world,config);
        String message = config.getString(path + ".Message");
        String command = config.getString(path + ".Command");
        List<Enemy> enemies = parseEnemies(path + ".Enemies",world,config);
        Menagerie.Doors doors = parseDoors(path + ".Doors",world,config);
        ...
        ItemStack giveItem = parseItemStack(path + ".GiveItem",config);
        if(NEncounter){
    ...
    private ObjectiveConditions parseCondition(String path,String world, FileConfiguration config) {
        if (config.isConfigurationSection(path + ".locationCondition")) {
            Location activationLoc = parseLocation(path + ".locationCondition.activationLoc",world,config);
            ...
            Location activationLoc = parseLocation(path + ".zoneCondition.activationLoc",world,config);
            ...
            Enemy enemy = parseEnemy(path + ".killCondition.enemy",world,config);
            ...
            List<Enemy> enemies = parseEnemies(path + ".multiKillCondition.enemies",world,config);
            return new ObjectiveConditions(enemies);
        }

        else if (config.isConfigurationSection(path + ".interactCondition")) {
            Location activationLoc = parseLocation(path + ".interactCondition.activationLoc",world,config);
            double activationRange = config.getDouble(path + ".interactCondition.activationRange", 0.0);
            ItemStack collectItemInHand = parseItemStack(path + ".interactCondition.collectItemInHand",config);
            ...
            Location activationLoc = parseLocation(path + ".collectCondition.activationLoc",world,config);
            ...
            ItemStack collectItemRecived = parseItemStack(path + ".collectCondition.collectItemRecived",config);
            return new ObjectiveConditions(activationLoc, activationRange, collectClickedMat, collectItemRecived);
        }
        return null;
    }
    private Menagerie.Doors parseDoors(String path,String world, FileConfiguration config) {
        if (config.isConfigurationSection(path)) {
            Location l1 = parseLocation(path + ".Location1",world,config);
            Location l2 = parseLocation(path + ".Location2",world,config);
    ...
    private Location parseLocation(String path,String world, FileConfiguration config) {
    ...
    private Enemy parseEnemy(String path,String world, FileConfiguration config) {
        Enemy enemy = null;
        if (config.isConfigurationSection(path)) {
                ...
                Location spawnLocation = parseLocation(path + ".SpawnLocation",world,config);
    ...
    private List<Enemy> parseEnemies(String path,String world, FileConfiguration config) {
        ...
        if (config.isConfigurationSection(path)) {
            for (String enemyKey : config.getConfigurationSection(path).getKeys(false)) {
                ...
                Location spawnLocation = parseLocation(enemyPath + ".SpawnLocation",world,config);
    ...
    private ItemStack parseItemStack(String path, FileConfiguration config) {
        if (config.isConfigurationSection(path)) {
            String materialName = config.getString(path + ".Material");
