...
import Abilities.PK_Abilities.Earth.SandBreath;
...
public class Methods {
				...
				for (Entity entity : loc.getWorld().getNearbyEntities(loc, range, range, range)) {
					if ((entity instanceof LivingEntity)) {
						if (affect == true) {
							...
							((LivingEntity) entity)
									.addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS, blinddur, 1));
						} else if (affect == false) {
							if (entity.getUniqueId() != player.getUniqueId()) {
								((LivingEntity) entity).addPotionEffect(
										new PotionEffect(PotionEffectType.SLOWNESS, slowdur, slowpower));
								((LivingEntity) entity).addPotionEffect(
										new PotionEffect(PotionEffectType.POISON, poisondur, poisonpower));
								((LivingEntity) entity)
										.addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS, blinddur, 1));
							}
						}
	...
	public static void FreezeTarget(Location location, int range, int rangeY, int maxy, long Duration, Material mat) {
		for (int i = 0; i < rangeY; i++) {
			for (Block b : GeneralMethods.getBlocksAroundPoint(location.add(0, i, 0), range)) {
				if ((b.getType().isAir() || WaterAbility.isWaterbendable(b.getType())
						|| EarthAbility.isEarthbendable(b.getType(), false, true, false)) &&
	 ...
	 * b.add(GeneralMethods.getDirection(b,
	 * player.getLocation().clone().add(0,1,0)).normalize().multiply(0.2));
				...
				new BukkitRunnable() {
					...
					public void run() {
						...
						for (Block b : GeneralMethods.getBlocksAroundPoint(location.subtract(0, 1, 0), range / 2)) {
							if (b.getY() == location.getY()) {
								if (WaterAbility.isEarth(b) || WaterAbility.isWater(b) || WaterAbility.isSand(b)
										|| WaterAbility.isSnow(b)) {
	...
	public static List<Location> BendableBlocksAnimation(List<Location> NearBlocks, Location playerloc, Material mat,
		...
		for (Location tloc : NearBlocks) {
			if (tloc.distance(playerloc) > 2) {
				...
				temploc.add(GeneralMethods.getDirection(temploc, playerloc.clone().add(0, 1, 0)).normalize()
						.multiply(speed));
				TempBlock tb1 = new TempBlock(temploc.getBlock(), mat);
				tb1.setRevertTime(100);
				ToAdd.add(temploc);
	...
	public static void stream(Location location, Vector dir, Player p, Ability abi, Material mat, int range,
			int damage) {
				...
				if ((entity instanceof LivingEntity) && entity.getUniqueId() != p.getUniqueId()) {
					...
					if (SandBreath.push == true) {
						...
						((LivingEntity) entity).addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS,
								SandBreath.DeBuffsDuration, SandBreath.DeBuffsPower, false, false));
	...
	public static void expansion(Location location, Player p, int times, int range, long duration, int sec,
			Material mat, boolean bendable, boolean waterbendalble) {
		Location loc = location.clone();
		int i = 0;
		int timeout = 0;
		do {
			...
			new BukkitRunnable() {
				public void run() {
					for (Block blocks : GeneralMethods.getBlocksAroundPoint(loc.clone().subtract(0, 1, 0),
							range * multi)) {
	...
	public static void displayLineBetweenPoints(Location source, Location destination, int particleAmount, Material mat,
			int duration) {
		int i = 0;
		int time = 0;
		do {
			new BukkitRunnable() {
				public void run() {
					...
					Vector direction = destination.clone().add(0, 1, 0).toVector()
							.subtract(source.clone().add(0, 1, 0).toVector()).normalize()
							.multiply(distance / particleAmount);
					...
					for (int i = 0; i < particleAmount; i++) {
						...
						// player.getWorld().spawnParticle(Particle.REDSTONE, particleLocation, 1, new
						// Particle.DustOptions(Color.fromRGB(50, 50, 50), 1));
	...
	public static Location findWaterSource(Player player, int radius) {
		...
		if (target != null) {
			if (WaterAbility.isWater(target) 
					|| WaterAbility.isWaterbendable(target.getType())
					|| WaterAbility.isPlantbendable(player, target.getType(), false)
					|| WaterAbility.isIcebendable(player, target.getType(), false)
					|| WaterAbility.isSnow(target.getType())
					|| target.getType() == Material.GRASS_BLOCK) {
