...
import static Plugin.AmonPackPlugin.ElementBasedOnSubElement;
import static Plugin.AmonPackPlugin.plugin;
...
public class Listeners implements Listener {
     ...
     * if
     * (InArenaRange(event.getEntity().getLocation(),A.getArenaLocation(),A.getRange
     * (),A.getRange())){
     ...
     * TextComponent.fromLegacyText("Odrodzenie za: "+countdown));
     ...
     * }
     * }
     * }.runTaskTimer(AmonPackPlugin.plugin, 0L, 20L); // Odświeżanie co 1 sekundę
     * (20 ticków)
     * }
     ...
     * Location nearestChestLocation =
     * newPvP.findNearestChestLocation(player.getLocation());
     * player.setCompassTarget(nearestChestLocation);
     * if (item.getType() == Material.COMPASS && item.getEnchantments().isEmpty()) {
     * int distance = (int)
     * Math.round(player.getLocation().distance(nearestChestLocation));
     * String message = "Najbliższa skrzynia znajduje się: " + distance +
     * " Bloków stąd.";
     * player.spigot().sendMessage(ChatMessageType.ACTION_BAR,
     ...
     * if (isNearWater(player,25,WMats) &&
     * playerinzone(player.getLocation(),Radius,PvPLoc)) {
     * player.removePotionEffect(PotionEffectType.CONFUSION);
     * }
     * }else if (ActEvent.equalsIgnoreCase("Chlod")){
     * if (isNearWater(player,25,FMats) &&
     * playerinzone(player.getLocation(),Radius,PvPLoc)) {
     ...
     * ItemStack item = player.getInventory().getItemInMainHand();
     * Location nearestChestLocation =
     * findNearestChestLocation(player.getLocation());
     * player.setCompassTarget(nearestChestLocation);
     * if (item.getType() == Material.COMPASS && item.getEnchantments().isEmpty()) {
     * int distance = (int)
     * Math.round(player.getLocation().distance(nearestChestLocation));
     ...
     * &&player.getLocation().distance(loc) < 2){
     ...
     * }}}
     * Commands.ExecuteCommandExample example = new
     * Commands.ExecuteCommandExample();
     * example.executeCommand("q event "+event.getPlayer().getName()
     * +" DailyPvP.jesli_PvP_RTP");
     ...
     * location.getWorld().spawnParticle(Particle.DUST, location, 5, 0.2, 0.2, 0.2,
     * 0.3, dustOptions);
     * location.getWorld().spawnParticle(Particle.PORTAL, location, 5, 0.2, 0.2,
     * 0.2, 0.3);
     * for (Entity HitBox : location.getWorld().getNearbyEntities(location, 1, 1,
     * 1)) {
     * for (Entity nearbyEntity : location.getWorld().getNearbyEntities(location,
     * 2.5, 1, 2.5)) {
     * if (nearbyEntity instanceof LivingEntity && !(nearbyEntity instanceof
     * ArmorStand) && !(nearbyEntity instanceof Player)) {
     * Vector direction =
     * nearbyEntity.getLocation().toVector().subtract(location.clone().subtract(0,
     * 3, 0).toVector()).normalize();
     * ((LivingEntity) nearbyEntity).damage(4);
     * location.getWorld().spawnParticle(Particle.EXPLOSION, location, 1, 0, 0, 0,
     * 0);
     * cancel();
     * }}
     * break;
     * }
     * if (origin.distance(location) > 25 ||
     ...
     * 0, 0, 0, 0, dustOptions);
     * particleLocation.getWorld().spawnParticle(Particle.PORTAL, particleLocation,
     * 1, 0, 0, 0, 0);
     * }
     * for (Entity hitBox : location.getWorld().getNearbyEntities(location, radius,
     * 1, radius)) {
     * if (hitBox instanceof LivingEntity && !(hitBox instanceof ArmorStand) &&
     * !(hitBox instanceof Player)) {
     * ((LivingEntity) hitBox).damage(3);
     * hitBox.setVelocity(direction.clone().add(new Vector(0,0.6,0).multiply(0.8)));
     * ((LivingEntity) hitBox).addPotionEffect(new
     * PotionEffect(PotionEffectType.SLOWNESS, 50, 3, false, false));
     * }
     * }
     ...
     * for (Entity entity :
     * GeneralMethods.getEntitiesAroundPoint(player.getLocation(), 4)) {
    ...
    public void onRightClick(PlayerInteractEvent event) {
        ...
        if (event.getAction() == Action.RIGHT_CLICK_BLOCK && event.getClickedBlock() != null) {
            ...
            // Location blacksmith = new Location(Bukkit.getWorld("Avatar-Main"), 1058, 20,
            // 694);
    ...
    public void BlockPlace(BlockPlaceEvent event) {
        ...
        /*
         * if (!AmonPackPlugin.BuildingOnArenas &&
         * event.getBlock().getWorld().equals(event.getPlayer().getWorld())){
         * for (Mine mine:Mining.ListOfMines) {
         * if(event.getBlock().getWorld().equals(mine.getLoc().getWorld())&&event.
         * getBlock().getLocation().distance(mine.getLoc())<= mine.getRadius()){
         * event.setCancelled(true);
         * break;
         * }}
         * for (Menagerie mena:ListOfAllMenageries) {
     ...
     * if (PerksMenager.PlayerConditionPerk(event.getPlayer(), "Unbreaking_Tools",
     * Perk.PerkType.PASSIVE)) {
     * event.setCancelled(true);
     * } else {
     ...
     * weapon.ExecuteEffects(event.getEntity(),player.getInventory().
     * getItemInMainHand(),player,true);
     * }
     * /*(PerksMenager.PlayerConditionPerk((Player)
     * event.getEntity(),"Anty_Fall_Dmg", Perk.PerkType.PASSIVE)) {
     * if (event.getCause() == EntityDamageEvent.DamageCause.FALL) {
     * event.setCancelled(true);
     * }
     * }
     ...
     * if (event.getEntityType() != EntityType.PLAYER) return;
     * if (PerksMenager.PlayerConditionPerk((Player) event.getEntity(),
     * "Farming_1_Jumping", Perk.PerkType.PASSIVE)) {
     * Block block = event.getBlock();
     * if (block.getType() == Material.FARMLAND || block.getBlockData() instanceof
     * Ageable) {
     ...
     * p.sendMessage(ChatColor.GREEN+ killedPlayer.getName() +
     * ChatColor.GREEN+" Został zabity przez Gracza: " + killer.getName());
     * }
     * for (String st : ChestList.get(0).getLoot()) {
     * killer.getInventory().addItem(Commands.QuestItemConfig(st));
     * }
     * if (fightParticipants.containsKey(killedPlayer) &&
     * playerBossBars.containsKey(killedPlayer)){
     ...
     * playerinzone(loc,re.getArenaS(),event.getEntity().getLocation())){
     * for (Player p:Bukkit.getOnlinePlayers()) {
     * if (p.getWorld().getName().equals(re.getBossLoc().getWorld().getName()) &&
     * playerinzone(loc,re.getArenaS(),p.getLocation())){
     ...
     * }}
     * p.sendMessage(ChatColor.RED+"[Ogłoszenie]  "+ChatColor.
     * DARK_PURPLE+"Udało wam się zabić Czempiona Króla! Rozkoszujcie się nagrodami!"
     * );
     * //p.teleport(RTP());
     ...
     * if
     * (event.getPlayer().getInventory().getItemInMainHand().isSimilar(MoonBlade)) {
     * for (Menagerie mena:ListOfAllMenageries) {
     * if (mena.IsInMenagerie(event.getPlayer().getLocation())) {
     * event.setCancelled(true);
     * break;
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        if (event.getCurrentItem() != null) {
            ...
            /*
             * if(Objects.equals(event.getInventory().getHolder(),
             * ForestMenager.ForestHolder)){
             * event.setCancelled(true);
             * if (!AmonPackPlugin.BuildingOnArenas) {
             * ItemStack item = event.getCurrentItem();
             * Forest forest =
             * ForestMenager.GetForestByLocation(event.getWhoClicked().getLocation());
             * if(forest!=null){
                ...
                if (event.getSlot() != 53) {
                    ...
                    if (!PlayerLevelMenager.CheckPlayerMagicEffectsCondition(effect, p)) {
                        p.sendMessage(ChatColor.RED + ChatColor.BOLD.toString()
                                + "Nie spełniasz wymaganych warunków, aby użyć tej Runy!");
                        return;
                    }
                    if (!CraftingMenager.HaveItems(p, false, effect.getCost())) {
                        p.sendMessage(
                                ChatColor.RED + ChatColor.BOLD.toString() + "Nie posiadasz wymaganych przedmiotów!");
                        return;
                    }
                    ...
                    if (effect.isMajorRune()) {
                        ...
                        if (CraftingMenager.HaveMajor(previewItem)) {
                            p.sendMessage(ChatColor.RED + ChatColor.BOLD.toString() + "Już wybrałeś " + ChatColor.ITALIC
                                    + "Większą Runę!");
                            return;
                        }
                    }
                    ...
                    List<MagicEffects> chosenEffects = CraftingMenager
                            .getEffectsFromItem(event.getInventory().getItem(53));
            ...
            if (Objects.equals(event.getInventory().getHolder(), PlayerLevelMenager.SkillDetails)) {
                ...
                if (event.getCurrentItem().getItemMeta().getDisplayName().contains("Poziom")
                        && event.getCurrentItem().getType() != Material.PLAYER_HEAD) {
                    LevelSkill.SkillType sktype = PlayerLevelMenager
                            .GetSkillTypeByMaterial(event.getCurrentItem().getType());
                    PlayerLevelMenager.ClaimReward(sktype, (Player) event.getWhoClicked(),
                ...
                if (sktype != null) {
                    ...
                    if (Level != null) {
                        PlayerLevelMenager.OpenSkillDetails(Level.GetSkillByType(sktype),
                ...
                if (ele != null) {
                    ...
                    if (element == ele) {
                        ...
                        event.getWhoClicked()
                                .sendMessage(ChatColor.RED + "Nie masz wybranego tego zywiołu! Twój zywioł to: "
                                        + playersBranch.getCurrentElement());
                    }
                }
                ...
                if (ClickedItem == Material.PAPER
                        && event.getCurrentItem().getItemMeta().getDisplayName().contains("Powrot")) {
                    AmonPackPlugin.levelsBending.OpenBendingSkillMenu(p.getName());
                } else if (ClickedItem.equals(Material.CHEST)) {
                    for (int i = 0; i <= 9; i++) {
                        if (CoreAbility.getAbility(bPlayer.getAbilities().get(i)) != null
                                && (CoreAbility.getAbility(bPlayer.getAbilities().get(i)).getElement()
                                        .equals(SelectedElement.getElement())
                                        || Objects.equals(
                                                ElementBasedOnSubElement(CoreAbility
                                                        .getAbility(bPlayer.getAbilities().get(i)).getElement()),
                    ...
                    SkillTree_Ability STA = SelectedElement.getAbilities().stream()
                            .filter(abi -> abi.getName()
             ...
             * if (newPvP.isInventoryEmpty(event.getView().getTopInventory()) &&
             * event.getClickedInventory().getType() == InventoryType.CHEST) {
             * event.setCancelled(true);
             * event.getView().getTopInventory().getLocation().getBlock().setType(Material.
             * AIR);
             * AmonPackPlugin.getPlayerMenager().AddPoints(LevelSkill.SkillType.COMBAT,
             * (Player) event.getWhoClicked(), fc.getExpgranted(), ChatColor.AQUA + "Exp:");
             * //Commands.ExecuteCommandExample example = new
             * Commands.ExecuteCommandExample();
             * //example.executeCommand("q event "+event.getWhoClicked().getName()
             * +" DailyPvP.punkt_PvP");
             * newPvP.LastFallChest.removeIf(b ->
             * b.getLocation().distance(event.getView().getTopInventory().getLocation()) <
             * 5);
             * }
             * }
             * }
     ...
     * if(QuestItems.ListOfAllQuestItems.contains(item) ||
     * item.getType()==Material.STONE){
     * event.setCancelled(true);
     * event.getPlayer().sendMessage(ChatColor.
     * RED+"Nie mozesz wyrzucac tego przedmiotu na pvp");
     * }
     * }
     * }
     */
     ...
     * for (Entity entity :
     * event.getPlayer().getWorld().getNearbyEntities(event.getPlayer().getLocation(
     * ), 10, 10, 10)) {
     * if (entity.getType() == EntityType.VINDICATOR || entity.getType() ==
     * EntityType.HUSK || entity.getType() == EntityType.SKELETON) {
     * event.setCancelled(true);
     * }}
     * if (event.getInventory().contains(new ItemStack(Material.BEDROCK))){
     * event.getInventory().remove(new ItemStack(Material.BEDROCK));
     * newPvP.SpawnMobs(event.getInventory().getLocation(),
     ...
     * Speed for 10 seconds (200 ticks)
     * }}
     * }}}
     */
     ...
     * if (hitEntity instanceof LivingEntity &&arrow.getCustomName()!=null&&
     * arrow.getCustomName().equalsIgnoreCase("MoonArrow")) {
     ...
     * /* if (playerinzone(event.getEntity().getLocation(),Radius,PvPLoc) ||
     * playerinzone(event.getEntity().getLocation(),Radius1,PvPLoc1)){
     ...
     * checkFights();
     * }else{
     * attacker.sendMessage(ChatColor.YELLOW+"Zaatakowałeś Gracza: "+ChatColor.RED+
     * victim.getName()+ChatColor.YELLOW+" ! Nie wylogywuj się!");
     * victim.sendMessage(ChatColor.YELLOW+"Zostałeś zaatakowany przez: "+ChatColor.
     * RED+attacker.getName()+ChatColor.YELLOW+" ! Nie wylogywuj się!");
     ...
     * event.getDamager().getType() == EntityType.VINDICATOR||
     * event.getDamager().getType() == EntityType.SKELETON||
     * event.getDamager().getType() == EntityType.HUSK){
     ...
     * Player player = event.getPlayer();
     * if(isInMultiworldsFolder(event.getFrom().getName()) &&
     * !isInMultiworldsFolder(event.getPlayer().getWorld().getName())){
    ...
    // if(event !=null &&
    // MagicEffects.AffectedAbilities.contains(event.getAbility())) {
    ...
    public void test(AbilityDamageEntityEvent event) {
        ...
        // CraftedWeapon weapon = CraftingMenager.getCraftedWeaponByItem(item);
        // if (weapon == null) return;
        // event.setDamage(weapon.ExecuteEffectsMagicDamage(player, event.getAbility(),
        // event.getDamage()));
        // }
        // }
        // }
