...
public class NoisySlash extends SoundAbility implements AddonAbility {
	...
	public NoisySlash(Player player) {
		...
		Particles.add(
				new BetterParticles(3, ParticleEffect.SPELL_MOB_AMBIENT, 0.0, 0.0, 0.0, Color.fromRGB(128, 128, 128)));
		...
		if (interval > 1) {
			...
			if (MainProjectile.getLocation().distance(origin) > 8) {
				...
				for (AbilityProjectile Projectile : Projectiles) {
					...
					if (location.distance(Projectile.getOrigin()) > 5
							|| !location.clone().add(0, 0.8, 0).getBlock().getType().isAir()
