...
public class SmokeSlash extends SmokeAbility implements AddonAbility {
	...
	public SmokeSlash(Player player) {
		...
		if (!this.bPlayer.isOnCooldown(getName()) && this.bPlayer.canBend(this)) {
			...
			MainOrb = new AbilityProjectile(player.getLocation().clone().getDirection(),
					player.getLocation().clone().add(0, 1, 0), origin, MainParticles, 1);
			...
			if (MainOrb != null) {
				...
				for (Entity entity : GeneralMethods.getEntitiesAroundPoint(location, 1)) {
					if ((entity instanceof LivingEntity) && entity.getUniqueId() != player.getUniqueId()) {
						SmokeSource SourceEnd = new SmokeSource(location.clone().add(0, 1, 0), SmokeDuration, 3, 1,
								player);
	...
	public String getDescription() {
		return "This ability creates smoke clouds! Slashes the target with a dark blade of smoke, inflicting blindness and poison.";
	}

	@Override
	public String getInstructions() {
		return "Left-click in close range to perform a smoke slash.";
	}
