...
public class SmokeCamouflage extends SmokeAbility implements AddonAbility {
	...
	private double Force = AmonPackPlugin.getAbilitiesConfig()
			.getDouble("AmonPack.Fire.Smoke.SmokeCamouflage.DashForce");
	private double ForceY = AmonPackPlugin.getAbilitiesConfig()
			.getDouble("AmonPack.Fire.Smoke.SmokeCamouflage.DashForce-Y");

	private long Duration = AmonPackPlugin.getAbilitiesConfig().getLong("AmonPack.Fire.Smoke.SmokeCamouflage.Duration");
	private int EffectsPower = AmonPackPlugin.getAbilitiesConfig()
			.getInt("AmonPack.Fire.Smoke.SmokeCamouflage.Effects-Power");
	...
	public void progress() {
		...
		if (AbilityState == State.READY) {
			...
			player.addPotionEffect(
					new PotionEffect(PotionEffectType.INVISIBILITY, 5, EffectsPower, false, false, false));
	...
	public String getDescription() {
		return "This ability uses smoke source! Pulls existing smoke, damaging enemies on its way. After cloud reach you, it blends your body into it making you invisible and stealthy.";
	}

	@Override
	public String getInstructions() {
		return "Sneak while looking at a smoke source to activate camouflage.";
	}
