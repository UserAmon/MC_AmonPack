...
public class WaterStrike extends WaterAbility implements AddonAbility {
    ...
    private Material sourceMaterial = Material.WATER;
    ...
    private List<WaterStrikeProjectile> projectiles = new ArrayList<>();

    public WaterStrike(Player player) {
        super(player);

        if (hasAbility(player, WaterStrike.class)) {
            return;
        }
    ...
    private void loadConfigAndUpgrades() {
        this.cooldown = AmonPackPlugin.getAbilitiesConfig().getLong("AmonPack.Water.WaterStrike.Cooldown", 6000L);
    ...
    public void progress() {
        ...
        if (state == State.CHARGING) {
            ...
            List<WaterStrikeProjectile> copy = new ArrayList<>(projectiles);
            for (WaterStrikeProjectile proj : copy) {
                proj.progress();
                if (proj.isDead()) {
    ...
    public class WaterStrikeProjectile {
        ...
        public WaterStrikeProjectile(Location startLoc, Vector initialDir, boolean isMini) {
        ...
        public void progress() {
            ...
            for (Entity entity : GeneralMethods.getEntitiesAroundPoint(loc, isMini ? 0.9 : 1.2)) {
                if (entity instanceof LivingEntity && !entity.getUniqueId().equals(player.getUniqueId())) {
                    ...
                    DamageHandler.damageEntity(target, dmg, WaterStrike.this);
        ...
        private void spawnSplitProjectiles() {
            ...
            for (int i = 0; i < splitCount; i++) {
                ...
                projectiles.add(new WaterStrikeProjectile(loc.clone(), splitDir, true));
            }
        }
