...
public class SandWave extends SandAbility implements AddonAbility, ComboAbility {
				...
				if (entity.getLocation().distance(proj) <= growth) {
						...
						if (entity.getUniqueId() != player.getUniqueId() && !Burrowed.contains(entity)) {
							if (entity.getLocation().getY() < proj.getY() + 1) {
								...
								((LivingEntity) entity).addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS,
										SandWave.DeBuffsDuration, SandWave.DeBuffsPower, false, false));
					...
					if (entity.getUniqueId() == player.getUniqueId()) {
						if (player.isSneaking()) {
							Vector forceDir = GeneralMethods.getDirection(player.getLocation(),
									proj.clone().add(0, 1, 0));
	...
	public SandWave(Player player, boolean IsEffect, Entity victim) {
		...
		((LivingEntity) victim).addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS, SandWave.DeBuffsDuration,
				SandWave.DeBuffsPower, false, false));
		((LivingEntity) victim).addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, SandWave.DeBuffsDuration,
				SandWave.DeBuffsPower, false, false));
	...
	public String getDescription() {
		return "Creates a rolling wave of sand that sweeps away enemies and buries them. Holding shift while near the wave, lets you ride it!";
	}
