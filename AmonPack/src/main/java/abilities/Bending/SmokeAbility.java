...
public abstract class SmokeAbility extends FireAbility implements SubAbility {
    ...
    public static SmokeSource UseSmokeSource(Player player, double range) {
        ...
        List<SmokeSource> sources = ListOfSources.stream()
                .filter(source -> source != null && source.getLocation().getWorld() != null
                        && Objects.equals(source.getLocation().getWorld(), location.getWorld())
                        && source.getLocation().distance(location) < range + 5)
